...
def processar_balanco(ano):
    ...
    # Ler
    df = pd.read_csv(
        f"Balanco_passivo_bruto/dfp_bpp_{ano}.csv",
        encoding='utf-8', sep=";"
    )
    ...
    df.to_csv(f"Balanco_passivo_tratado/dfp_bpp_{ano}_filtrado.csv", index=False, encoding='utf-8', sep=";")
...
balanco_completo.to_csv("Balanco_passivo_tratado/dfp_bpp_completo.csv", index=False, encoding='utf-8', sep=";")
