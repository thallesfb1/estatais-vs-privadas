...
if __name__ == "__main__":
    ...
    for ano in range(2014, 2024):
        ...
        dre = pd.read_csv(f"DRE_tratado/dfp_dre_{ano}_filtrado.csv",              sep=";", encoding="utf-8")
        bpa = pd.read_csv(f"Balanco_ativo_tratado/dfp_bp_{ano}_filtrado.csv",     sep=";", encoding="utf-8")
        bpp = pd.read_csv(f"Balanco_passivo_tratado/dfp_bpp_{ano}_filtrado.csv",  sep=";", encoding="utf-8")
        ...
        print(f"  Financeiro: {len(kpis_fin)} empresas | Energia: {len(kpis_en)} empresas")

    # Consolidar e salvar
    # Ao salvar, garantir que string vazia não vire NaN
    pd.concat(kpis_fin_list, ignore_index=True).to_csv(
        "KPIs/kpis_financeiro.csv", index=False, sep=";", encoding="utf-8", na_rep=''
    )
    pd.concat(kpis_en_list, ignore_index=True).to_csv(
        "KPIs/kpis_energia.csv", index=False, sep=";", encoding="utf-8", na_rep=''
    )
    pd.concat(zscores_fin_list, ignore_index=True).to_csv(
        "KPIs/zscores_financeiro.csv", index=False, sep=";", encoding="utf-8", na_rep=''
    )
    pd.concat(zscores_en_list,  ignore_index=True).to_csv(
        "KPIs/zscores_energia.csv",    index=False, sep=";", encoding="utf-8", na_rep=''
    )

    print("\nConcluído!")
    kpis_en_completo = pd.concat(kpis_en_list, ignore_index=True)
    print(kpis_en_completo.groupby(['ano', 'tipo']).size().reset_index(name='empresas'))